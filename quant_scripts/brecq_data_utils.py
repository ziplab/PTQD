...
def save_inp_oup_data(model: QuantModel, layer, cali_images, cali_t, cali_y,
    ...
    for i in range(int(cali_images.size(0) / batch_size)):
        cur_inp, cur_out = get_inp_out(cali_images[i * batch_size:(i + 1) * batch_size],cali_t[i * batch_size:(i + 1) * batch_size],cali_y[i * batch_size:(i + 1) * batch_size])
...
def save_inp_oup_data_block(model: QuantModel, layer: QuantModule, cali_images, cali_t, cali_y,
    ...
    for i in range(int(cali_images.size(0) / batch_size)):
        cur_inp0, cur_inp1, cur_out = get_inp_out(cali_images[i * batch_size:(i + 1) * batch_size],cali_t[i * batch_size:(i + 1) * batch_size],cali_y[i * batch_size:(i + 1) * batch_size])
...
def save_grad_data(model: QuantModel, layer: QuantModule, cali_images, cali_t, cali_y,
    ...
    for i in range(int(cali_images.size(0) / batch_size)):
        cur_grad = get_grad(cali_images[i * batch_size:(i + 1) * batch_size],cali_t[i * batch_size:(i + 1) * batch_size],cali_y[i * batch_size:(i + 1) * batch_size])
...
class GetLayerInpOut:
    ...
    def __call__(self, cali_images, cali_t, cali_y):
        self.model.eval()
        ...
        with torch.no_grad():
            try:
                _ = self.model(cali_images.to(self.device),cali_t.to(self.device),cali_y.to(self.device))
