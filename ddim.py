...
from scripts.brecq_quant_layer import QuantModule
...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        iterator = tqdm(time_range, desc='DDIM Sampler', total=total_steps, disable=True) ## disable tqdm here
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        # coef = (1. - a_prev - sigma_t**2).sqrt() - a_prev.sqrt() * sqrt_one_minus_at / a_t.sqrt()
        if noise_dropout > 0.:
            noise = torch.nn.functional.dropout(noise, p=noise_dropout)
        # print(torch.norm(a_prev.sqrt() * pred_x0 + dir_xt) / torch.numel(dir_xt) / torch.pow(sigma_t, 2)[0].item())
...
class DDIMSampler_quantCorrection_imagenet(object):
    def __init__(self, model, num_bit, correct=False, schedule="linear", **kwargs):
        ...
        self.kt_dict = np.load('correct_data/250steps1eta1.5scale_brecq_w4a8/imagenet_w4a8_kt.npy', allow_pickle=True).item()
        self.sigmaq_dict = np.load('correct_data/250steps1eta1.5scale_brecq_w4a8/imagenet_w4a8_t_std_dict.npy', allow_pickle=True).item()
        self.index_bias = np.load('correct_data/250steps1eta1.5scale_brecq_w4a8/imagenet_w4a8_idx_bias.npy')
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        e_t = e_t / (1+self.kt(t))  ## correct correlated part of quantization error
        
        e_t = e_t - self.saved_bias(index)  ## correct bias
